...
impl NegotiationStore {
    ...
    async fn submit_offer(&self, id: u64, sender: &str, amount: f64) -> Result<OfferResult, String> {
        ...
        let is_initiator = neg.initiator == sender;
        let is_counterparty = neg.counterparty == sender;
    ...
    async fn withdraw(&self, id: u64, sender: &str) -> Result<Negotiation, String> {
        let mut negotiations = self.negotiations.write().await;
        let neg = negotiations.get_mut(&id).ok_or("Negotiation not found.")?;
        ...
        if neg.initiator != sender && neg.counterparty != sender {
            return Err("You are not a participant in this negotiation.".into());
        }
    ...
    async fn get_pending_for_user(&self, user: &str) -> Vec<Negotiation> {
        self.negotiations
            .read()
            .await
            .values()
            .filter(|n| {
                n.status == NegotiationStatus::WaitingForOffers
                    && (n.initiator == user || n.counterparty == user)
            })
            .cloned()
            .collect()
    }
...
impl CommandHandler for OfferHandler {
    ...
    async fn execute(&self, message: &BotMessage) -> AppResult<String> {
        ...
        match self.store.submit_offer(id, &message.source, amount).await {
...
impl CommandHandler for WithdrawHandler {
    ...
    async fn execute(&self, message: &BotMessage) -> AppResult<String> {
        ...
        match self.store.withdraw(id, &message.source).await {
            Ok(neg) => Ok(format!(
...
impl CommandHandler for DealsHandler {
    ...
    async fn execute(&self, message: &BotMessage) -> AppResult<String> {
        let pending = self.store.get_pending_for_user(&message.source).await;
